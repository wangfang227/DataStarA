...
c = con.cursor()
#e = pd.read_sql_query("Select ExtractedBodyText From Emails where ExtractedBodyText like '%President%' and ExtractedBodyText like '%2016%' limit 10",con)
...
len(cs)
# cs is the combined text string

type(e.ExtractedBodyText)

str(e.ExtractedBodyText)
...
'''
#cs is the raw string, to tokenize, we need the string variable
for i in range(len(text)):
    tokeni=nltk.wordpunct_tokenize(text[i])
    texti=nltk.Text(tokeni)
    texti.concordance("money")
    texti.concordance("election")
...
text1.dispersion_plot(["united states", "money", "friend", "china"])

text1.common_contexts(["money", "china"])
...
text1.count("united states")

#in the text1, each email is treated as one word
...
tokens=nltk.wordpunct_tokenize(text[1])
...
tfidf = TfidfVectorizer(tokenizer=tokenize, stop_words='english',max_df=0.99, min_df=0.01,ngram_range=(1, 2))
tfidf
